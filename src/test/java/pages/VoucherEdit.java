 ...
 *
 */
public class VoucherEdit extends BaseWeb{
	WebDriver driver;
	...
	public VoucherEdit(WebDriver driver) {
		this.driver = driver;
	}
	...
	@FindBy(how = How.XPATH, using = "//*[@id=\"root\"]/div[1]/div[2]/div[2]/div/div/div/div[2]/table/tbody/tr/td[2]")
	@CacheLookup
	WebElement code;
	
	@FindBy(how = How.XPATH, using = "//*[@id=\"root\"]/div[1]/div[2]/div[2]/div/div/div/div[1]/button[2]")
	@CacheLookup
	WebElement saveVoucherButton;
	...
	public void inputName() {
		...
		(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(name)).sendKeys("EditAuto"+getTime());
	}
	...
	public void searchNewVoucher() throws InterruptedException {
		(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(searchField)).clear();
		(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(searchField)).sendKeys("EditAuto"+whatTime);
		Thread.sleep(2000);
	}
	...
	public boolean isVoucherManualTrue(String a) {
		if((new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(code)).getText().contains(a)) {
			return true;
		}else {
			return false;
		}
	...
	public boolean isVoucherAutomaticallyTrue() {
		if((new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(code)).isDisplayed()) {
			System.out.println((new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(code)).getText());
			return true;
		}else {
			System.out.println((new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(code)).getText());
			return false;
	...
	public void getEditUrl() {
		driver.get(url+"/vouchers/Edit/186");
	}
	...
	public void getVoucherPage() {
		driver.get(url+"/vouchers");
	}
	
	public boolean isEditNameWork() {
		if((new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(tableFirstList)).getText().contains("EditAuto"+whatTime)) {
			return true;
		}else {
			return false;
		}
	...
	public void clickSaveVoucherButton() throws InterruptedException {
		(new WebDriverWait(driver, 10)).until(ExpectedConditions.visibilityOf(saveVoucherButton)).click();;
